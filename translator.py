...
def translate(text):
    ...
    terms.append(Term(line_ctr, "jmp", ".start", 0, 0, 0))
    line_ctr += 1
    ...
    for line in text.split("\n"):
        ...
        if data_section:
            ...
            if len(tokens[1]) == 3 and tokens[1][0] == '\"' and tokens[1][2] == '\"':
                terms.append(Term(line_ctr, 'data', ord(tokens[1][1]), 0, AddrMode.LIT.value, 0))
            else:
                terms.append(Term(line_ctr, 'data', tokens[1], 0, AddrMode.LIT.value, 0))
        else:
            ...
            terms.append(Term(line_ctr, tokens[0], tokens[1], tokens[2], 0, 0))
    ...
    for i in range(len(terms)):
        ...
        if terms[i].arg1 in labels.keys():
            terms[i] = Term(terms[i].line, terms[i].operation,
                            labels[terms[i].arg1], terms[i].arg2,
                            AddrMode.PTR, 0)
        elif terms[i].arg1 in variables.keys():
            terms[i] = Term(terms[i].line, terms[i].operation,
                            variables[terms[i].arg1], terms[i].arg2,
                            AddrMode.PTR, 0)
        ...
        if terms[i].arg2 in labels.keys():
            ...
            terms[i] = Term(terms[i].line, terms[i].operation,
                            terms[i].arg1, int(terms[i].arg2),
                            terms[i].mode1, AddrMode.LIT)
